...
uint8_t XPT2046_Touch_Calibrate ( uint8_t LCD_Mode ) 
{
    ...
    strType_XPT2046_Coordinate strCrossCoordinate[4], strScreenSample[4];
	  ...
	  strType_XPT2046_Calibration CalibrationFactor;
		...
		{ 
			...
			LCD_SetColors(BLUE, BLACK);
			ILI9341_DrawRectangle(12, 12, (uint16_t)(18 * (i + 1)), 8, 1);
	...
	ILI9341_DrawRectangle((LCD_X_LENGTH >> 1) - 32, (LCD_Y_LENGTH >> 1) - 24, 64, 48, 0);
	ILI9341_DrawRectangle((LCD_X_LENGTH >> 1) - 20, (LCD_Y_LENGTH >> 1) - 8, 40, 16, 1);
...
Failure:
	...
	ILI9341_DrawLine((LCD_X_LENGTH >> 1) - 30, (LCD_Y_LENGTH >> 1) - 30,
	                 (LCD_X_LENGTH >> 1) + 30, (LCD_Y_LENGTH >> 1) + 30);
	ILI9341_DrawLine((LCD_X_LENGTH >> 1) + 30, (LCD_Y_LENGTH >> 1) - 30,
	                 (LCD_X_LENGTH >> 1) - 30, (LCD_Y_LENGTH >> 1) + 30);
  XPT2046_DelayUS ( 1000000 );		
...
/**
  * @brief  设置触摸扫描模式，并按需启动图形十字校准。
  * @note   当前工程为控制程序体积，不再读写外部 SPI Flash 保存校准参数。
  * @note   forceCal 为 0 时仅切换扫描模式并使用默认参数；为 1 时本次上电期间使用新校准参数。
  * @param  LCD_Mode: 要校正触摸参数的液晶模式
  * @param  forceCal: 是否强制重新校正参数
  * @retval 无
  */	
...
uint8_t XPT2046_Get_TouchedPoint ( strType_XPT2046_Coordinate * pDisplayCoordinate, strType_XPT2046_TouchPara * pTouchPara )
{
  ...
  {    
		int16_t x = ( ( pTouchPara[LCD_SCAN_MODE].dX_X * strScreenCoordinate.x ) + ( pTouchPara[LCD_SCAN_MODE].dX_Y * strScreenCoordinate.y ) + pTouchPara[LCD_SCAN_MODE].dX );
		int16_t y = ( ( pTouchPara[LCD_SCAN_MODE].dY_X * strScreenCoordinate.x ) + ( pTouchPara[LCD_SCAN_MODE].dY_Y * strScreenCoordinate.y ) + pTouchPara[LCD_SCAN_MODE].dY );

		if (x < 0) x = 0;
		if (y < 0) y = 0;
		if (x >= (int16_t)LCD_X_LENGTH) x = (int16_t)LCD_X_LENGTH - 1;
		if (y >= (int16_t)LCD_Y_LENGTH) y = (int16_t)LCD_Y_LENGTH - 1;

		/* Current board touch panel is mounted opposite to LCD scan mode 3. */
		pDisplayCoordinate ->x = (int16_t)LCD_X_LENGTH - 1 - x;
		pDisplayCoordinate ->y = (int16_t)LCD_Y_LENGTH - 1 - y;
